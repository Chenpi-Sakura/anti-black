...
triggers: ["风控", "黑灰产", "fraud", "舞弊", "刷单", "薅羊毛", "撞库", "爬虫", "代理IP", "设备风险", "营销风险", "支付风险", "商家作弊", "内容刷量", "BREAK", "risk analysis", "anti-abuse", "trust and safety", "case analysis", "风控case", "反作弊", "反欺诈"]
tools: []
plan_template: ["了解案件场景", "区分已知/假设/证据缺口", "映射BREAK风险ID", "生成分析结果"]
---
...
## 无追问规则（继承 CORE）
- 场景不明确时（"分析一下这个情况"），默认从用户提供的信息推断最高风险方向的业务场景
- 不允许反问"你具体遇到了什么"——基于已有信息推进并标注假设

## 输出结构
- 案件摘要：已知事实 | 假设 | 证据缺口
- 攻击者画像：角色（置信度）| 资源偏好（依据）
- 滥用链路分析
- BREAK 风险映射表
- 信号与风控建议
- 结论与下一步
