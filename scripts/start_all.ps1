...
Write-Host "[1/4] 正在启动 API 服务..." -ForegroundColor Green
...
Write-Host "[2/4] 正在启动 爬虫底层 API 与 调度器..." -ForegroundColor Green
...
Write-Host "[3/4] 正在启动 数据采集推流 (Publisher)..." -ForegroundColor Green
...
Write-Host "[4/4] 正在启动 核心处理引擎 (Daemon)..." -ForegroundColor Green
...
Write-Host "您将看到 5 个新的 PowerShell 窗口，分别负责不同的流水线节点。" -ForegroundColor Yellow
