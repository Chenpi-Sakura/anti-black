...
class SlangLearner:
    ...
    async def _validate_candidate_with_llm(self, candidate: SlangCandidate) -> tuple[bool, str]:
        ...
        # ---- 四段式 Prompt：人设 + 核心任务重定义 + 核心防误判 + 对抗性样本 ----
...
**【核心任务重定义：打破暗语洁癖，关注交易特征】**

你正在担任安全风控系统的特征提取引擎。
请注意，我们要抓取的 is_valid_slang（黑产词）不仅仅指隐晦暗语。

在黑灰产交易（特别是账号买卖、引流、代客刷单）中，有大量
字面意思直白、但高度集中于违规交易场景的词汇，这些必须被
判定为 is_valid_slang: true！

✅ 必须判定为 True 的情况（高危交易特征）：
- 明确的账号交易客体描述（如："万粉号"、"百粉号"、"实名号"、"白号"）
- 明确的规避行为描述（如："换绑即可绝不找回"、"实名已解"）
- 普通词在真实业务数据中绝大多数伴随违规买卖出现时

你的思考逻辑：
只要这个词在真实数据中绝大多数情况都是伴随违规行为出现的，
它就是有价值的风控特征，必须输出 is_valid_slang: true。
不要因为"字面意思不够隐蔽"就否定。

**【严禁收录分词碎片和无意义短语】（致命重要）**

你必须识别并拒绝那些仅仅是句子随机切分出来的"语法碎片"或"临时动作拼接"！
如果一个候选词：
1. 本身不具备固定的名词、动词或专有名词属性（例如："刚刚图一扣"、"一半了"、"的话就"）
2. 只是普通口语中偶然连在一起的字（哪怕它出现在了黑产对话里）
**你必须判定 is_valid_slang = false！**

你的任务是提取具有特定指代意义的"特征词"（如"万粉号"、"出个"、"连西"），
绝对不是记录骗子说的每一句口水话。如果这个词换个正常人、在正常语境下
根本不可能作为一个"固定短语"说出口，请直接毙掉它。

**【判断优先级（请严格按此顺序判定）】**：
1. 先用【核心防误判规则】中第 4 条的拒绝清单（标签/商务词/画像/碎片/官方词/反诈词）判断 → 命中即 false。
2. 再判断【严禁收录分词碎片】→ 是"刚刚图一扣"这类拼接碎片 → false。
3. 最后判断【核心任务重定义】→ 字面直白但**在数据中绝大多数伴随违规行为**的（如"出个"+"号"成
   完整的交易短语）→ true。
**注意：步骤 3 的优先级高于步骤 2。** 也就是说"出个"虽然短，但因为它在数据中只
与交易场景共现（如"出个号"、"出个万粉号"），仍属于有效黑产特征词，不适用"分词碎片"规则。
...
2. 如果该词是常见的日常动词/形容词（如："买卖"、"加好友"），哪怕它出现在违规帖子里，它也不是黑话。但是，如果该词在数据中极度高频伴随违规行为出现（如"出抖号"、"万粉号"、"换绑即可绝不找回"），即使字面直白也应当视为有效黑产特征。
...
3. 对抗性负例 (test_negative_cases)：造 2 个用来验证你正则精确度的测试用例。注意：
   ...
   b. **【关键】如果你的正则有前缀列表（如 `(?:出|收|售|购)`），负例中绝对不能出现这些前缀字中的任意一个，无论什么含义！**
      → 因为正则不区分 "出售"的"出"和 "运营出"的"出"，re.search 见到"出"字就会匹配。
      → 正确做法：选一个不含这些前缀字的完全不同的场景。
