...
class DouYinCrawler(AbstractCrawler):
    ...
    async def launch_browser_with_cdp(
        ...
        try:
            ...
            # CDP failed - do NOT fall back to Playwright, that requires chromium installed
            # Instead, raise so caller knows CDP mode is unavailable
            raise RuntimeError(f"CDP mode failed, cannot fall back to standard mode (Playwright chromium not installed): {e}") from e
